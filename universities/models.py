...
class UserDashboard(models.Model):
    ...
    def update_subscription(self, amount_paid, monthly_price=1000):
        from django.utils import timezone
        from datetime import timedelta
        from decimal import Decimal
        
        # Ensure amount is a Decimal for consistent math
        if not isinstance(amount_paid, Decimal):
            amount_paid = Decimal(str(amount_paid))
        if not isinstance(monthly_price, Decimal):
            monthly_price = Decimal(str(monthly_price))
        
        print(f"=== UPDATE_SUBSCRIPTION CALLED ===")
        print(f"  amount_paid={amount_paid}, monthly_price={monthly_price}")
        print(f"  Before update: status={self.subscription_status}, end_date={self.subscription_end_date}")
        print(f"  Before update: total_paid={self.total_paid}, months_subscribed={self.months_subscribed}, is_verified={self.is_verified}")
        
        # Initialize months_to_add
        months_to_add = 0
        
        # Only update if amount is positive
        if amount_paid > 0:
            # Update total_paid
            self.total_paid = Decimal(str(self.total_paid)) + amount_paid
            ...
            if self.subscription_end_date and self.subscription_end_date > today:
                ...
                old_end = self.subscription_end_date
                self.subscription_end_date = self.subscription_end_date + timedelta(days=30 * months_to_add)
                print(f"  Extended end_date from {old_end} to {self.subscription_end_date}")
            else:
                # If subscription is expired or doesn't exist, set from today
                self.subscription_end_date = today + timedelta(days=30 * months_to_add)
